  ...
  try {
    ...
    if (permission === 'default') {
      ...
      if (newPermission === 'granted') {
        logToConsole('INFO', 'Разрешение на уведомления получено');
        setupNotificationListener();
        return true;
      } else {
        logToConsole('WARN', 'Пользователь отказал в уведомлениях');
        return false;
      }
    } else if (permission === 'granted') {
      logToConsole('INFO', 'Уведомления уже разрешены');
      setupNotificationListener();
      return true;
    }
    ...
    logToConsole('ERROR', 'Ошибка инициализации уведомлений', error);
    return false;
  }
}

// Настройка прослушивания обновлений статуса
function setupNotificationListener() {
  // Обновляем Service Worker с ID водителя
  if (navigator.serviceWorker && navigator.serviceWorker.controller) {
    navigator.serviceWorker.controller.postMessage({
      type: 'SET_DRIVER_ID',
      driverId: registrationState.data.phone // Используем телефон как ID
    });
  }
  ...
  logToConsole('INFO', 'Слушатель обновлений статуса настроен');
}
...
function handleStatusUpdate(update) {
  ...
  // Обновляем статус на текущем шаге если он активен
...
function showBrowserNotification(update) {
  if (Notification.permission !== 'granted') return;
  
  const options = {
    body: formatNotificationBody(update),
    icon: '/reg_driver_ULN/icons/icon-192x192.png',
    badge: '/reg_driver_ULN/icons/icon-72x72.png',
    tag: `status-update-${update.registrationId}`,
    renotify: true,
    vibrate: [200, 100, 200],
    data: {
      update: update,
  ...
  const notification = new Notification(formatNotificationTitle(update), options);
  ...
  notification.onclose = () => {
    logToConsole('INFO', 'Уведомление закрыто', { registrationId: update.registrationId });
  };
}

// Показ уведомления в приложении
function showAppNotification(update) {
  const notificationText = formatAppNotification(update);
  
  // Показываем sticky-уведомление
  showStickyNotification(notificationText, 'info');
...
function formatNotificationTitle(update) {
  const titles = {
  ...
  return titles[update.newStatus] || '📋 Обновление статуса';
}

// Форматирование тела уведомления
function formatNotificationBody(update) {
  ...
  if (update.problemType && (update.newStatus === 'Проблема с товаром' || 
                             update.newStatus === 'Проблема с документами' || 
                             update.newStatus === 'Отказ в приемке')) {
    body += `\nПричина: ${update.problemType}`;
  }
  
  if (update.fio) {
    body = `Уважаемый ${update.fio},\n${body}`;
  }
  
  return body;
}

// Форматирование уведомления в приложении
function formatAppNotification(update) {
  let message = '';
  
  switch(update.newStatus) {
    case 'Назначены ворота':
      message = `🚪 <b>НАЗНАЧЕНЫ ВОРОТА</b>\n\n`;
      message += `Вам назначены ворота №<b>${update.assignedGate || 'не указаны'}</b>\n`;
      message += `📍 Если ворота свободны - занимайте, если заняты - ожидайте`;
      break;
      
    case 'Документы готовы к выдаче':
      message = `📄 <b>ДОКУМЕНТЫ ГОТОВЫ</b>\n\n`;
      message += `Ваши документы готовы к выдаче\n`;
      message += `Пожалуйста, проследуйте к месту подачи документов`;
      break;
      
    case 'Отказ в приемке':
      message = `❌ <b>ОТКАЗ В ПРИЕМКЕ</b>\n\n`;
      message += `Приносим глубочайшие сожаления\n`;
      if (update.problemType) {
        message += `Причина: ${update.problemType}`;
      }
      break;
      
    case 'Нет в графике':
      message = `⏰ <b>ВНЕ ГРАФИКА</b>\n\n`;
      message += `Вы приехали вне графика\n`;
      message += `Рекомендуем связаться с поставщиком`;
      break;
      
    case 'Проблема с товаром':
    case 'Проблема с документами':
      message = `⚠️ <b>ПРОБЛЕМА</b>\n\n`;
      message += `Возникла ${update.newStatus.toLowerCase()}\n`;
      if (update.problemType) {
        message += `Тип: ${update.problemType}\n`;
      }
      message += `Ожидайте дополнительной информации`;
      break;
      
    default:
      message = `📋 <b>ОБНОВЛЕНИЕ СТАТУСА</b>\n\n`;
      message += `Новый статус: ${update.newStatus}`;
  }
...
// Sticky-уведомление (остается пока не закрыто)
function showStickyNotification(message, type = 'info') {
  const stickyContainer = document.getElementById('sticky-notifications');
  if (!stickyContainer) return;
  
  const notificationId = 'sticky-' + Date.now();
  const notification = document.createElement('div');
  notification.id = notificationId;
  notification.className = `sticky-notification ${type}`;
  notification.innerHTML = `
    <div class="sticky-content">${message}</div>
    <button class="sticky-close" onclick="closeStickyNotification('${notificationId}')">✕</button>
  `;
  
  stickyContainer.appendChild(notification);
  
  // Автоматически закрываем через 30 секунд
  setTimeout(() => {
    closeStickyNotification(notificationId);
  }, 30000);
}

function closeStickyNotification(id) {
...
// Добавление в историю уведомлений
function addToNotificationHistory(update) {
  try {
    ...
    logToConsole('ERROR', 'Ошибка сохранения в историю уведомлений', error);
  }
}
...
function updateNotificationBadge() {
  try {
    ...
    // Также обновляем заголовок вкладки
    if (unreadCount > 0) {
      document.title = `(${unreadCount}) Регистрация водителей`;
    } else {
      document.title = 'Регистрация водителей';
    }
    
  } catch (error) {
    logToConsole('ERROR', 'Ошибка обновления бейджа', error);
  }
}

// Модальное окно с деталями статуса
function showStatusDetailsModal(update) {
  const modalHtml = `
    <div class="modal-overlay" onclick="closeModal(event)">
      <div class="modal" onclick="event.stopPropagation()">
        <div class="modal-header">
          <h3 class="modal-title">${formatNotificationTitle(update)}</h3>
          <button class="modal-close" onclick="closeModal(event)">✕</button>
        </div>
        <div class="modal-body">
          <div class="status-details">
            <div class="status-item">
              ...
              <span class="status-value">${update.newStatus}</span>
            </div>
            ${update.assignedGate ? `
            <div class="status-item">
              <span class="status-label">Ворота:</span>
              <span class="status-value">${update.assignedGate}</span>
            </div>` : ''}
            ${update.supplier ? `
            <div class="status-item">
              <span class="status-label">Поставщик:</span>
              <span class="status-value">${update.supplier}</span>
            </div>` : ''}
            ${update.problemType ? `
            <div class="status-item">
              <span class="status-label">Причина:</span>
              <span class="status-value">${update.problemType}</span>
            </div>` : ''}
            <div class="status-item">
              <span class="status-label">Время:</span>
              <span class="status-value">${new Date(update.timestamp).toLocaleString('ru-RU')}</span>
            </div>
          </div>
          ${getStatusInstructions(update)}
        </div>
        <div class="modal-footer">
          <button class="btn btn-primary" onclick="markNotificationAsRead('${update.registrationId}')">
  ...
  markNotificationAsRead(update.registrationId);
}

function getStatusInstructions(update) {
  switch(update.newStatus) {
    case 'Назначены ворота':
      return `
        <div class="info-box">
          <p><b>Инструкция:</b></p>
          <p>📍 Если ворота свободны - можете занимать</p>
          <p>⏱️ Если заняты - ожидайте</p>
          <p>⚠️ Не забудьте поставить противооткатный башмак</p>
        </div>
      `;
    ...
    case 'Документы готовы к выдаче':
      return `
        <div class="info-box">
          <p><b>Инструкция:</b></p>
          <p>📍 Проследуйте к месту подачи документов</p>
          <p>📋 Возьмите с собой документы на груз</p>
        </div>
      `;
    ...
    case 'Отказ в приемке':
      return `
        <div class="warning-box">
          ...
          <p>📞 Свяжитесь с вашим поставщиком</p>
          <p>🔄 Уточните причину отказа</p>
          <p>⏱️ Ожидайте дальнейших инструкций</p>
        </div>
      `;
...
function markNotificationAsRead(registrationId) {
  try {
    const history = JSON.parse(localStorage.getItem('notification_history') || '[]');
    
    history.forEach(notification => {
      if (notification.update.registrationId === registrationId) {
    ...
    logToConsole('ERROR', 'Ошибка пометки уведомления как прочитанного', error);
  }
}
...
function showNotificationHistory() {
  try {
    ...
    let html = `
      <div class="modal-overlay" onclick="closeModal(event)">
        <div class="modal" onclick="event.stopPropagation()" style="max-width: 600px;">
          <div class="modal-header">
            <h3 class="modal-title">📋 История уведомлений</h3>
            <button class="modal-close" onclick="closeModal(event)">✕</button>
          </div>
          <div class="modal-body">
            <div style="margin-bottom: 20px;">
    ...
    if (history.length === 0) {
      html += '<div class="empty-state">Нет уведомлений</div>';
    } else {
      history.forEach(notification => {
        ...
        html += `
          <div class="notification-item ${readClass}" onclick="showStatusDetailsModal(${JSON.stringify(update)})">
            <div class="notification-header">
              <span class="notification-title">${formatNotificationTitle(update)}</span>
              <span class="notification-time">${time}</span>
            </div>
            <div class="notification-body">
              ${formatNotificationBody(update).replace(/\n/g, '<br>')}
            </div>
          </div>
        `;
      });
    ...
    logToConsole('ERROR', 'Ошибка показа истории уведомлений', error);
    showNotification('Ошибка загрузки истории уведомлений', 'error');
  }
}
...
function clearNotificationHistory() {
  if (confirm('Очистить всю историю уведомлений?')) {
    ...
    updateNotificationBadge();
    closeModal();
    showNotification('История уведомлений очищена', 'success');
  }
}
...
function updateSuccessStepWithStatus(update) {
  ...
  // Добавляем блок с текущим статусом
  const statusHtml = `
    <div class="current-status" style="margin-top: 20px; padding: 15px; background: #e3f2fd; border-radius: 8px; border-left: 4px solid #2196f3;">
      <p style="margin: 0; font-weight: 500; color: #1565c0;">Текущий статус: ${update.newStatus}</p>
      ${update.assignedGate ? `<p style="margin: 5px 0 0 0; color: #333;">Ворота: ${update.assignedGate}</p>` : ''}
      ${update.problemType ? `<p style="margin: 5px 0 0 0; color: #c62828;">Причина: ${update.problemType}</p>` : ''}
      <p style="margin: 5px 0 0 0; font-size: 12px; color: #666;">
        Обновлено: ${new Date(update.timestamp).toLocaleString('ru-RU')}
      </p>
    </div>
  `;
  
  // Проверяем, не добавлен ли уже блок со статусом
  if (!container.querySelector('.current-status')) {
...
function closeStickyNotification(id) {
  ...
  if (notification) {
    ...
    setTimeout(() => {
      ...
      // Скрываем контейнер если нет уведомлений
      const stickyContainer = document.getElementById('sticky-notifications');
      if (stickyContainer && stickyContainer.children.length === 0) {
  ...
  try {
    ...
    if (history.length === 0) {
      ...
      history.forEach(notification => {
        ...
        html += `
          <div class="notification-item ${readClass}" onclick="showStatusDetailsModal(${JSON.stringify(update).replace(/"/g, '&quot;')})">
            <div class="notification-header">
              <span class="notification-title">${formatNotificationTitle(update)}</span>
              <span class="notification-time">${time}</span>
            </div>
